function datalabel=read_datalabel(G, rawdir,starttime,endtime)
filelist=findfiles(rawdir,G.LABEL.DATALABEL_FILENAME);
...
for r=1:row
    ...
    labelno=labelno+1;
    datalabel(labelno).starttimestamp=starttime;
    datalabel(labelno).endtimestamp=endtime;
    datalabel(labelno).startmatlabtime=convert_timestamp_matlabtimestamp(G,starttime);
    datalabel(labelno).endmatlabtime=convert_timestamp_matlabtimestamp(G,endtime);
