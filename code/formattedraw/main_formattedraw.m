...
%    if G.RUN.FRMTRAW.TOSFILETIME==1, fprintf('...saving tos file time'); tosFileTime=read_tosFileTime(G,indir,starttimestamp,endtimestamp);R.tosFileTime=tosFileTime;saveTosTimestamps(pid,sid,tosFileTime);end
...
%     if G.RUN.FRMTRAW.EMA==1, fprintf('...ema');R.ema=read_ema(G,indir,starttimestamp,endtimestamp);end
    ...
    if isfield(G.RUN.FRMTRAW,'DATALABEL') && G.RUN.FRMTRAW.DATALABEL==1, fprintf('...datalabel'); R.datalabel=read_datalabel(G,indir,starttimestamp,endtimestamp);end;    
