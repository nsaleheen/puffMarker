...
for i=1:m
    if touched(i)==0;
       ob=x(i,:);
%        D=dist(ob(2:n),x(:,2:n));
%        nearThreshold_ind=pointsAroundWithinThreshold(ob(2:n),x(:,2:n));
%        D=distHaverSine(ob(2:n),x(nearThreshold_ind,2:n));
       D=distHaverSine(ob(2:n),x(:,2:n));
       ind=find(D<=Eps);
...
function ind=pointsAroundWithinThreshold(i,x)
    %ind=find(abs(i(1,1)-x(:,1))<0.004 & abs(i(1,2)-x(:,2))<0.004);
    ind=find(abs(i(1,1)-x(:,1))<0.01 & abs(i(1,2)-x(:,2))<0.01);
end