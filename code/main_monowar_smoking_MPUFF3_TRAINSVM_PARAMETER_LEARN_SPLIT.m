function main_monowar_smoking_MPUFF3_TRAINSVM_PARAMETER_LEARN_SPLIT()
close all
clear all
G=config();
...
outfile=[G.DIR.DATA G.DIR.SEP 'weka' G.DIR.SEP 'weka_train_new_output_split.txt'];
modelfile=[G.DIR.ROOT G.DIR.SEP 'weka' G.DIR.SEP 'weka_train_new_model_split.model'];
parameterfile=[G.DIR.DATA G.DIR.SEP 'weka' G.DIR.SEP 'weka_train_new_parameter_split.csv'];

max_detail=-inf;
id=fopen(parameterfile,'w');
for c=pow2(-10):pow2(0.5):pow2(5)
    for g=pow2(-10):pow2(.5):pow2(5)
        weka_train_SMO_PARAMETER_smoking_split(wekafile,modelfile,outfile,c,g);
