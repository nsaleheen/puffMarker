%function [b0,b1,tau1,sse,r2]=curve_fit_activation(G,pid,sid,wlarge,wsmall,type)
function [res,samples,matlabtimes]=curve_fit_recovery(G,pid,sid,wlarge,wsmall,type)
samples=[];matlabtimes=[];
...
for i=1:length(P.rr.window.p1)
    if P.rr.window.mark(i)==type
        v2=P.rr.window.v2(i);
        p2=P.rr.window.p2(i);
        stime=P.rr.avg.matlabtime(v2);
        etime=P.rr.avg.matlabtime(p2);
        if type==1,
...
%        matlabtime=matlabtime(sx:sy);
        ...
        if length(sample)<60 , continue;end;
        sample=diff(sample);
        matlabtime(end)=[];
        ...
        samples=[samples sample];
        matlabtimes=[matlabtimes matlabtime];
        [b,g]=recovery(matlabtime*24*60*60,sample,0);
        now=now+1;
        res(now).b=b;
        res(now).g=g;
