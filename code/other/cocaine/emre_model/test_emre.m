...
function [valid,mse_activity,mse_cocaine,dose_estimate]=test_emre(CN,Basesol,tau1sol,tau2sol)
Basesol=0.9*Basesol/1000;
...
datatest{1}.samples=CN.rr.sample(ind);

if length(datatest{1}.time)<4,
    valid=0;
    return;
end
x=fminunc(@(x) recovery2robust([Basesol tau1sol x(3)], datatest,35),[Basesol tau1sol  datatest{1}.time(1)]);
 

mse_activity=recovery2robust(x,datatest,35);
...
x=fmincon(@(x) recovery3conrobust([Basesol tau1sol tau2sol x(4:5)] ,datatest,35),[Basesol tau1sol tau2sol datatest{1}.time(1) 1],...
[],[],[],[],zeros(5,1),[],[]);

%mse_activity=recovery3con(x,datatest);
mse_cocaine=recovery3conrobust(x,datatest,35);
