function rr=find_activity(G,pid,sid,MODEL,rr)
...
now=length(rr);
for i=1:length(C.valley_ind)
    starttimestamp=C.timestamp(C.valley_ind(i));
    endtimestamp=C.timestamp(C.peak_ind(i+1));
    if C.Q9_smooth(C.valley_ind(i))>baserr || C.Q9_smooth(C.peak_ind(i+1))<baserr,continue;end;
