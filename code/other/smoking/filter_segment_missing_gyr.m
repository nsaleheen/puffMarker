function filter_segment_missing_gyr(G, pid,sid,INDIR,OUTDIR,ACC)
...
for i=1:2
    ...
    for s=1:length(P.wrist{i}.gyr.segment.starttimestamp)
        stime=P.wrist{i}.gyr.segment.starttimestamp(s)-1000;
        etime=P.wrist{i}.gyr.segment.endtimestamp(s)+1000;
        for id=IDS
            ind=find(P.sensor{id}.timestamp>=stime & P.sensor{id}.timestamp<=etime);
            if length(ind)<ACC*G.SENSOR.ID(id).FREQ*(etime-stime)/1000
                P.wrist{i}.gyr.segment.missing(s)=1;
                break;
            end
        end
    end
