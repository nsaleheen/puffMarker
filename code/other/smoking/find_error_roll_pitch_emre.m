function A=find_error_roll_pitch_emre(R)
A=[];
for i=1:2
    [A{i}.rp{1},A{i}.rp{2}]=find_roll_pitch_sample(R,i);
    for s=1:length(A{i}.rp{1}.sample)
        ...
        A{i}.puff_nonpuff{1}.error(s)=A{i}.rp{1}.p_error(s)/A{i}.rp{1}.np_error(s);
        A{i}.puff_nonpuff{2}.error(s)=A{i}.rp{2}.p_error(s)/A{i}.rp{2}.np_error(s);
        A{i}.puff_nonpuff{3}.error(s)=(A{i}.rp{1}.p_error(s)+A{i}.rp{2}.p_error(s))/(A{i}.rp{1}.np_error(s)+A{i}.rp{2}.np_error(s));
    ...
    for j=1:3
        ...
        if j==1, type='r '; elseif j==2, type=' p'; else, type='rp'; end;
...
if i==1,
    saveas(h,'C:\Users\smh\Desktop\smoking_fig\emre_roll_pitch_histogram\ROC_lefthand.png','png');
else
    saveas(h,'C:\Users\smh\Desktop\smoking_fig\emre_roll_pitch_histogram\ROC_righthand.png','png');    
end
end
