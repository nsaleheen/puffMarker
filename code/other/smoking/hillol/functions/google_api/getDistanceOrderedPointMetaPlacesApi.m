function poi = getDistanceOrderedPointMetaPlacesApi( lat, lon, typeArr )
...
apiUrl = sprintf('https://maps.googleapis.com/maps/api/place/search/xml?location=%f,%f&types=%s&sensor=false&rankby=distance&key=%s', lat, lon, typesStr, apiKey);
strXml = urlread(apiUrl);
%strXml = native2unicode(strXml,'UTF-8');
%strXml = strXml(ismember(double(strXml),0:255));
%strXml(5518:5518) = 97; %getDistanceOrderedPointMetaPlacesApi(35.3504, -89.9052, [7 9 15 38 60 61 79 67])
