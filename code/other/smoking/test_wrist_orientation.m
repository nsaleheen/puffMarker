function P=test_wrist_orientation(G,P,time,pid,sid)
%fprintf('...correct_orientation');
Hand=generate_orientation_data(G,P,time);
for i=1:2
    len=length(Hand(i,1).valid);
    P.wrist{i}.CI.valid=Hand(i,1).valid;P.wrist{i}.CI.timestamp=Hand(i,1).timestamp;
    P.wrist{i}.CI.actual=zeros(1,len);P.wrist{i}.CI.predict=zeros(1,len)-1;
    [ac,pr,prob]=run_orientation_model(G,Hand,time,'CI',i,pid,sid);
    i1=find(Hand(i,1).valid==0);P.wrist{i}.CI.predict(i1)=pr;
end
end
