function main_nazir_smoking_E3F2_TRAIN_THRESHOLD_ROC_POSTPROCESSING()
close all
clear all
G=config();
...
ylim([0.5,1]);
disp('abc');
filename=['E:\smoking_memphis_nazir\data\Memphis_Smoking_Lab\smoking_fig' G.DIR.SEP  'rip_wrist_feature_ROC'];
...
%     fprintf('a=1: %d; prob>=th: %d\n', length(find(A==1)), length(find(prob>=TH)));
    a0=length(find(A==0));
    a1=length(find(A==1));
    pbL=length(find(prob<TH));
    pbG=length(find(prob>=TH));
