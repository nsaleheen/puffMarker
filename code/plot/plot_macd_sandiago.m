...
hhh=0;h=[];
hhh=hhh+1;h(hhh)=plot_signal(P.rr.matlabtime,P.rr.sample,'g.',1,0);
h(hhh)=plot_signal(P.rr.avg.matlabtime,P.rr.avg.t600,'r--',2,0);legend_text{1}='Moving Avg(RR Interval)';
hhh=hhh+1;
...
%        continue;
%    end;
    h(2)=plot_signal(P.rr.avg.matlabtime(p1),P.rr.avg.t600(p1),'bo',4,0);
    h(2)=plot_signal([P.rr.avg.matlabtime(p1) P.rr.avg.matlabtime(p1)],[500 1800],'k:',2,0);legend_text{2}='Cross point';
...
h(3)=plot_signal(P.rr.macd.matlabtime,P.rr.macd.M1,'b-',2,1500,1.5); legend_text{3}='MACD Line';
h(4)=plot_signal(P.rr.macd.matlabtime,P.rr.macd.M2,'r-',2,1500,1.5);legend_text{4}='Signal Line';
h(5)=plot_signal([P.rr.macd.matlabtime(1),P.rr.macd.matlabtime(end)],[1500,1500],'k-',2);legend_text{5}='Base Line';
...
time='02/13/2012 12:30:00';timestamp=convert_time_timestamp(G,time);t1=convert_timestamp_matlabtimestamp(G,timestamp);
time='02/13/2012 14:00:00';timestamp=convert_time_timestamp(G,time);t2=convert_timestamp_matlabtimestamp(G,timestamp);

%xlim([t1,t2]);
set(gca,'FontSize',20);
legend(h,legend_text,'Interpreter', 'none');
xlabel('Time','FontSize',20);
ylabel('RR Interval         MACD                                 ','FontSize',20);
legend(h,legend_text,'Interpreter', 'none');
