function iplot4_monowar_pv_puff_episode(G, pid,sid, INDIR, slist, plotCres,eno)
    shiftCress=0;
    ...
    for s=slist
        ...
        h(i+1)=plot_signal(B.sensor{s}.matlabtime,B.sensor{s}.sampleNorm,color{rem(i,length(color))+1},1,offset);
        if s==1,
            plot_signal(B.sensor{s}.peakvalley.matlabtime(2:2:end),B.sensor{s}.peakvalley.sampleNorm(2:2:end),'b.',1,offset);
        hold on;
        for i=1:length(B.cress.puff_peak)
            x=B.cress.puff_peak(i);
            plot_signal(B.sensor{1}.peakvalley.matlabtime(x),B.sensor{1}.peakvalley.sampleNorm(x),'go',5,offset);
        end
...
%	legend(h,legend_text,'Interpreter', 'none');
    xlabel('Time');
    ylabel('Magnitude');
    xlim([stime-2/(24*60), etime+2/(24*60)]);
