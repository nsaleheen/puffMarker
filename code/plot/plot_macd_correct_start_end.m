function plot_macd_correct_start_end(G,pid,sid,M,L,R)
h=[];
...
h(2)=plot_signal(P.rr.smoothtime,P.rr.(['t' num2str(M.W)]),'y-',2,0);
legend_text{2}=['Moving Average (' num2str(M.W) ' seconds)'];
h(2)=plot_signal([P.rr.smoothtime(1),P.rr.smoothtime(end)],[0,0],'k-',2,0);
[mvm1,mvm2,pos1]=find_macd_intersect(P.rr.(['t' num2str(M.W)]),M.SLOW,M.FAST,M.SIGNAL);
[mvl1,mvl2,pos2]=find_macd_intersect(P.rr.(['t' num2str(L.W)]),L.SLOW,L.FAST,L.SIGNAL);
[mvr1,mvr2,pos3]=find_macd_intersect(P.rr.(['t' num2str(R.W)]),R.SLOW,R.FAST,R.SIGNAL);

h(3)=plot_signal(P.rr.smoothtime,mvm1,'b-',2,0);
h(3)=plot_signal(P.rr.smoothtime,mvm2,'g-',2,0);
sind=[];eind=[];
for i=1:length(pos1)
    [x,ind]=min(abs(P.rr.smoothtime(pos2)-P.rr.smoothtime(pos1(i))));
    sind(i)=pos2(ind);
    [x,ind]=min(abs(P.rr.smoothtime(pos3)-P.rr.smoothtime(pos1(i))));
    eind(i)=pos3(ind);    
end
for i=1:length(sind)-1
    stime=P.rr.smoothtime(sind(i));
    etime=P.rr.smoothtime(eind(i+1));
    h(5)=plot_signal(P.rr.smoothtime(sind(i)),P.rr.(['t' num2str(M.W)])(sind(i)),'go',4);
    legend_text{5}='Start Window';
    h(6)=plot_signal(P.rr.smoothtime(eind(i+1)),P.rr.(['t' num2str(M.W)])(eind(i+1)),'ro',4);
    legend_text{6}='End Window';
end
%..h(3)=plot_signal(P.rr.smoothtime(pos),P.rr.t600(pos),'go',2,0);
%legend_text{3}=['MACD'];
