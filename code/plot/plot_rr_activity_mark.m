function [h,legend_text]=plot_rr_activity_mark(G,pid,sid,pda)
hh = figure();
...
h(1)=plot_signal(B.sensor{G.SENSOR.R_ACLXID}.matlabtime,10*(B.sensor{G.SENSOR.R_ACLXID}.sample)+1000,'b.',1);
...
%coc_mark=plot_pdamark(G,pid,sid,'formattedraw');
    ptime=datenum([pda.date ' ' pda.drugstarttime]);
    plot([ptime,ptime],ylim,'k-','LineWidth',2);
    
    hold on;
    tx=[pda.drugamount ' ' char(10) '(' pda.drugactual ')'];
