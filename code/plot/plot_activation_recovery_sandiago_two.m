function [h,legend_text]=plot_activation_recovery_sandiago_two(G,pid,sid,wlarge,wsmall,field)
hh = figure();
...
h(3)=plot_signal(P.acl.avg60.matlabtime,val60,'b-',1,1400);legend_text{3}='Activity Indicator (ACCL)';
%h(3)=plot_signal(P.acl.avg120.matlabtime,val120,'b-',1,1000);legend_text{3}='varince of magnitude (accel)';

h(4)=plot_signal([P.acl.matlabtime(1),P.acl.matlabtime(end)],[threshold,threshold],'r-',1,1400);legend_text{4}='Activity Threshold';
...
coc_mark_lab=plot_adminmark(G,pid,sid,'formattedraw',[450,2400],1200);
...
title('Lab Study Data');
set(xx,'FontSize',24);
xlabel('Time','FontSize',20);
ylabel('           RR Interval                       ACCL.                   ','FontSize',20);
