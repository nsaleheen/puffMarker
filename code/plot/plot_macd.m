function plot_macd(G,pid,sid,w,s,f,t)
h=[];
...
h(2)=plot_signal(P.rr.smoothtime,P.rr.(['t' num2str(w)]),'y-',2,0);
legend_text{2}=['Moving Average (' num2str(w) ' seconds)'];
h(2)=plot_signal(P.rr.smoothtime,P.rr.t600,'r-',2,0);
legend_text{2}=['Moving Average (' num2str(w) ' seconds)'];
h(2)=plot_signal([P.rr.smoothtime(1),P.rr.smoothtime(end)],[0,0],'k-',2,0);
[mv1,mv2,pos]=find_macd_intersect(P.rr.(['t' num2str(w)]),s,f,t);
h(3)=plot_signal(P.rr.smoothtime,mv1,'b-',2,0);
h(3)=plot_signal(P.rr.smoothtime,mv2,'g-',2,0);
