...
% 		plot_custom(G,pid,sid,'svm_output','plot_svm_output_episode',[],'smokinglabel','gyrmag',[2,1],'segment_gyr','map',[2,1],'selfreport','svm');
        ...
        [selfreport_timestamp, selfreport_matlabtime]= get_first_selfreport_time(G, data);
         A=[];
         B=[];
         
         fprintf('%s, %s\n', num2str(selfreport_matlabtime), num2str(selfreport_timestamp));
		...
		for i=1:2      
            ...
            for j=1:length(puff_times)
                diff = (data.sensor{1}.peakvalley_new_3.matlabtime(pind(j))-selfreport_matlabtime)*60*24;
                A=[A;diff];
               
%                  if abs(diff) < 80
                    fprintf('%s, %s, %s\n', num2str(diff), num2str(data.sensor{1}.peakvalley_new_3.matlabtime(pind(j))), num2str(data.sensor{1}.peakvalley_new_3.timestamp(pind(j))));
%                  end
            end
            fprintf('\n');
        end
