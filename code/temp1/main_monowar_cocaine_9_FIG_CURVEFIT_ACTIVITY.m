function main_monowar_cocaine_9_FIG_CURVEFIT_ACTIVITY()
clear all
G=config();
...
for i=1:20
    ...
    s=E{2}.N{i}.window_time(3);
    t=E{2}.N{i}.window_time(4);
    ind=find(E{2}.N{i}.rr.avg.timestamp>=s & E{2}.N{i}.rr.avg.timestamp<=t);
    if length(ind)<180, continue;end;
    if length(find(E{2}.N{i}.rr.avg.t60(ind)>1200)), continue;end;
    if max(E{2}.N{i}.rr.avg.t60(ind))<800, continue;end;
    hold on;plot(E{2}.N{i}.rr.avg.t60(ind),'b-');
    c=c+1;
    N{c}=E{2}.N{i};
    if c==5, break;end;
end
...
MA=[];MC=[];D=[];O=[];
pid='p02';sid='s06'; %lab
%pid='p01';sid='s10'; %field

ppid=1;ssid=10;

load NIDA_tau
who=0;
[h,legend_text]=plot_activation_recovery_sandiago_two(G,pid,sid,600,600,2);
