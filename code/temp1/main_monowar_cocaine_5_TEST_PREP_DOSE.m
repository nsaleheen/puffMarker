...
function main_monowar_cocaine_5_TEST_PREP_DOSE(G,PS_LIST,DOSE)
...
%        [n,rr]=get_emre_recovery_activity(G,pid,sid,0);if ~isempty(n),N=[N,n];end
        [c,rr]=get_emre_recovery_new_test_dose(G,pid,sid,1,DOSE); if ~isempty(c), C=[C,c];end; RR=[RR,rr];
        [n,rr]=get_emre_recovery_new_test_dose(G,pid,sid,0,DOSE);if ~isempty(n),N=[N,n];end
    end
