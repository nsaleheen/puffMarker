function main_nida_monowar_window_filtering()
clear all
G=config();
G=config_run_nida(G);
...
valid_ind=0;
for i=61:length(v)
    count=length(find(N.acl.avg.sample(i-60:i)>N.acl.threshold));
    if count/60>.25, 
        break;
    else valid_ind=i-60;
    end
end
H=max(N.rr.avg.sample)-min(N.rr.avg.sample);
h=max(N.rr.avg.sample(1:valid_ind))-min(N.rr.avg.sample(1:valid_ind));
if H*.95>h,
    v=[];t=[];
end
end
